            ...
            if articles:
                ...
                for article in articles:
                    ...
                    if not existing:
                        ...
                        new_count += 1
                        new_articles_for_user.append(article)
                
                print(f"    ✅ Fetched {len(articles)} articles ({new_count} new)")
            else:
...
# Now send only new articles from the last 48 hours
print("\n" + "=" * 60)
print("Sending new articles from last 48 hours...")
...
for user in users:
    ...
    # Get new articles from last 48 hours
    cursor.execute('''
    ...
    print(f"  Found {len(articles)} new articles from last 48 hours")
    
    if articles:
        # Send header message
        header_message = f"📰 **New Articles (Last 48 Hours)** ({len(articles)} articles)\n\n"
        ...
        print(f"  No new articles found from last 48 hours")

conn.close()

print("\n" + "=" * 60)
print("✅ New articles sent to all users (no duplicates)")
